...
const System = {
                ...
                <div style="display:flex; align-items:center; gap: 15px;">
                    ...
                    <div>
                        ...
                        <div style="font-size: 11px; opacity: 0.5;">${
                          isInstalled ? "Nainstalováno" : "Zdarma"
                        }</div>
                    </div>
                </div>
  ...
  renderCalendar: () => {
        ...
        <div class="calendar-wrapper">
            ...
            <div class="calendar-header">
                <div class="month-nav">
                    ...
                    } <span style="font-weight:300; opacity:0.7;">${viewYear}</span></h2>
                ...
                <div class="today-display" onclick="System.openApp('calendar')"> <!-- Kliknutím reset na dnešek -->
                    ...
                    <span style="font-weight:bold;">${realDate.getDate()}. ${
      realDate.getMonth() + 1
    }.</span>
                </div>
            </div>
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      let hasEvent =
        rawData && (typeof rawData === "string" || rawData.length > 0);
      ...
      html += `
                <div class="${classes}" onclick="System.openCalendarModal(${i}, ${
        viewMonth + 1
      }, ${viewYear})">
  ...
  renderEventList: (events) => {
    ...
    if (events.length === 0) {
      list.html(
        '<div style="text-align:center; opacity:0.5; padding:20px;">Žádné plány</div>'
      );
      return;
    }
  ...
  renderSettings: () => {
   ...
   API: {
        // 1. UNIVERZÁLNÍ GRAF (Chart.js)
        // Plugin pošle: { type: 'chart', targetId: 'newsChart', config: { ...Chart.js config... } }
        renderChart: (payload) => {
            setTimeout(() => {
                ...
                // Globální registr grafů, abychom je mohli ničit při překreslení
                if (!window.activeCharts) window.activeCharts = {};

                // Pokud graf na tomto plátně už existuje, zničíme ho (prevence glitchů)
        ...
        // 2. UNIVERZÁLNÍ NOTIFIKACE (SweetAlert2)
        // Plugin pošle: { type: 'notification', title: 'Chyba', text: 'Nemáš peníze', icon: 'error' }
        ...
        // 3. AKTUALIZACE TEXTU/HTML (bez překreslení celé stránky)
        // Plugin pošle: { type: 'updateElement', targetId: 'bank-balance', content: '$50,000' }
        updateElement: (payload) => {
            const el = $(`#${payload.targetId}`); // Používáme jQuery selektor
            if (el.length) {
                if (payload.isHtml) el.html(payload.content);
                else el.text(payload.content);
                
                // Volitelně animace při změně
                if (payload.animate) {
